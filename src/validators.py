...
def check_multipath_conf():
    ...
    if not os.path.exists(mfile):
        ff(name, "/etc/multipath.conf file not found", "1D506D89")
    ...
    if not defaults:
        return ff(name, "defaults section not found", "1D8C438C")
    ...
    if get_os() == "ubuntu":
        ...
        for d in defaults[1]:
            if 'checker_timer' in d:
                ct = True
        if not ct:
            ff(name, "defaults section missing 'checker_timer'", "FCFE3444")
    else:
        ct = False
        for d in defaults[1]:
            if 'checker_timeout' in d:
                ct = True
        if not ct:
            ff(name, "defaults section missing 'checker_timeout'", "70191A9A")

    # Check devices section
    devices = filter(lambda x: x[0] == 'devices', mconf)
    if not devices:
        return ff(name, "Missing devices section", "797A6031")
    devices = devices[0][1]
    dat_block = None
    ...
    for _, device in be:
        ...
        if bdict['vendor'] == 'DATERA.*':
            dat_block = bdict
    if not dat_block:
        ff(name, "No Datera blacklist_exceptions section found", "09E37E51")
